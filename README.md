<!-- Banner -->
<p align="center">
  <img src="https://capsule-render.vercel.app/api?type=waving&color=0:0f2027,100:2c5364&height=200&section=header&text=Nishant%20Sorout&fontSize=40&fontColor=ffffff&animation=fadeIn" />
</p>

<h2 align="center">🚀 Cloud & DevOps Engineer</h2>

<p align="center">
  AWS • Terraform • Docker • CI/CD • Cloud Security • Kubernetes
</p>

---

## 👨‍💻 About Me

- ☁️ Building scalable and secure AWS infrastructure  
- ⚙️ Automating deployments using CI/CD pipelines  
- 🐳 Working with containers (Docker, ECS, EKS)  
- 🔐 Focused on cloud security, IAM, WAF, and KMS  
- 📊 Implementing monitoring with Prometheus, Grafana & CloudWatch  

---

## 🛠️ Tech Stack

### ☁️ Cloud & Platforms
<p>
  <img src="https://skillicons.dev/icons?i=aws,gcp" />
</p>

### ⚙️ DevOps & IaC
<p>
  <img src="https://skillicons.dev/icons?i=terraform,docker,kubernetes,jenkins,githubactions" />
</p>

### 📊 Monitoring & Observability
<p>
  <img src="https://skillicons.dev/icons?i=grafana,prometheus" />
</p>

### 💻 Scripting & OS
<p>
  <img src="https://skillicons.dev/icons?i=linux,bash" />
</p>

---

## 🧰 AWS Services

EC2 • ECS • EKS • S3 • CloudFront • Route 53 • IAM • RDS • ALB • WAF • KMS • CloudWatch • SNS • SQS

---

## 💼 Experience

### 🚀 Cloud & DevOps Engineer — Bellblaze Technologies
- Built AWS infrastructure using Terraform  
- Worked on ECS, EKS, ALB, CloudFront, Route 53  
- Implemented IAM, WAF, KMS security  
- Designed CI/CD pipelines using GitHub Actions  
- Set up monitoring & logging (CloudWatch, Grafana)  

### ☁️ AWS Cloud Engineer Trainee — TATA STRIVE (Airbus)
- Hands-on with EC2, S3, IAM, ECS, RDS  
- Docker-based deployments  
- Auto-scaling and load balancing  

---

## 🏆 Certifications
- AWS Certified Cloud Practitioner  

---

## 📫 Connect With Me

<p>
  <a href="https://linkedin.com/in/nishantsorout08">
    <img src="https://skillicons.dev/icons?i=linkedin" />
  </a>
  <a href="mailto:nishantsorout07@gmail.com">
    <img src="https://skillicons.dev/icons?i=gmail" />
  </a>
</p>

---

## 📊 GitHub Stats

<p align="center">
  <img src="https://github-readme-stats.vercel.app/api?username=YOUR_USERNAME&show_icons=true&theme=tokyonight" />
  <img src="https://github-readme-streak-stats.herokuapp.com/?user=YOUR_USERNAME&theme=tokyonight" />
</p>

---

<p align="center">
  ⚡ Building scalable cloud systems | Automating everything ⚡
</p>
